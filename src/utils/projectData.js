const ProjectsData = [
  {
    thumbnail: "/projects/Aparna/thumbnail.jpg",
    slug: 'aparna',
    cover: "/projects/Aparna/Cover.jpg",
    title: "Aparna",
    location: "Gachibowli",
    plan: "Luxuria",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit etma, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  },
  {
    thumbnail: "/projects/Aparna-2/thumbnail.jpg",
    slug: 'aparna-2',
    cover: "/projects/Aparna-2/Cover.jpg",
    title: "Aparna 2",
    location: "Gachibowli",
    plan: "Cozy",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit amet, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  },
  {
    thumbnail: "/projects/Aparna-3/thumbnail.jpg",
    slug: 'aparna-3',
    cover: "/projects/Aparna-3/Cover.jpg",
    title: "Aparna 3",
    location: "Gachibowli",
    plan: "Comfort",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit amet, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  },
  {
    thumbnail: "/projects/Aparna/thumbnail.jpg",
    slug: 'aparna-4',
    cover: "/projects/Aparna/Cover.jpg",
    title: "Aparna 4",
    location: "Gachibowli",
    plan: "Luxuria",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit amet, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  },
  {
    thumbnail: "/projects/Aparna-2/thumbnail.jpg",
    slug: 'aparna-5',
    cover: "/projects/Aparna-2/Cover.jpg",
    title: "Aparna 5",
    location: "Gachibowli",
    plan: "Cozy",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit amet, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  },
  {
    thumbnail: "/projects/Aparna-3/thumbnail.jpg",
    slug: 'aparna-6',
    cover: "/projects/Aparna-3/Cover.jpg",
    title: "Aparna 6",
    location: "Gachibowli",
    plan: "Comfort",
    type: "3 BHK",
    heading_1: "Lorem ipsum dolor sit amet, consetetur",
    desc_1: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et",
    heading_2: "Lorem ipsum dolor sit amet, consetetur",
    desc_2: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren,",
    carousel: [
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      },
      {
        img: "/projects/Aparna/Demo_photo.jpg"
      }
    ],
    team: [
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      },
      {
        name: "Roshan Agarwal",
        desc: "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod",
        img: "/projects/1.png"
      }
    ]
  }
]

export function getAllProjectIds() {
  return ProjectsData.map((project) => {
    return {
      params: {
        slug: project.slug
      },
    };
  });
}

export function getProjectData(slug) {
   const proj = ProjectsData.filter(x => x.slug === slug)
   return {
    proj
   };
}

export default ProjectsData;